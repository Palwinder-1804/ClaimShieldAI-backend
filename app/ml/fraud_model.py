...
class FraudDetector:
    ...
    async def predict(self, features: Dict[str, Any], claim_id: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            # Run in thread pool with strict timeout of 3.0 seconds to prevent blocking/hanging
            await asyncio.wait_for(asyncio.to_thread(log_to_mlflow_sync), timeout=3.0)
        except Exception as e:
            # Fail silently to avoid breaking the core pipeline when MLflow is not running
            logger.warning(f"MLflow logging bypassed or timed out: {e}")
